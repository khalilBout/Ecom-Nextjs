    ...
    <Link href={`/shop/${product._id}`} className="w-full relative group">
      <div className=" relative overflow-y-hidden ">
        ...
        <div className=" absolute top-6 left-8">
          {globalStoke === 0 ? (
            ...
            <div className="border border-rose-200 rounded-md w-[92px] h-[35px] text-primeColor flex justify-center items-center text-base font-semibold hover:bg-black duration-300 cursor-pointer">
              {product.type}
            </div>
          )}
        </div>
      ...
      <div className="max-w-80 py-4 flex flex-col gap-1 border-[1px] border-t-0 px-4">
        ...
        <div className="flex justify-between items-center">
          <p className="text-rose-400 font-bold text-[16px] ">
            {product.category}
          </p>
        ...
        <div className="text-[#767676] hover:text-primeColor text-sm font-normal flex items-center  gap-2 hover:cursor-pointer pb-1 duration-300 w-full">
          Color:
          {listOfColors?.map((item, index) => (
            <span
              key={index}
