...
const ProductOrder = ({ item, deleteModel, handelUpdateModel }) => {
  ...
  product?.listModels.map((item) => {
    const stokeOfModel = item.size.reduce(
      (total, item) => item.stoke + total,
      0
    );
    if (stokeOfModel > 0) colorData.push(item);
  });
            ...
            <div className="">
              ...
              <div className="py-1">
                {colorData.map((c, index) => (
                  <button
                    key={index}
                    className={`relative w-[20px] h-[20px] border-[1px] border-black m-1
          ${
                    ...
                    newSizeSelect?.theSize === size.theSize
                      ...
                      `}
                          ...
                          >
                            ...
                            <span className="text-blue-700">{size?.stoke}</span>
                          </button>
                        </div>
                      ))}
                    </div>
              ...
              <div className="flex flex-col justify-between">
                ...
                <div className="">
                  <p className="text-l font-semibold text-gray-600">
                    Qt: <span className="text-red-400 ">{item.quantity}</span>
                  </p>
                  {sizeSelected?.stoke < item.quantity ? (
                    <p className="text-red-400 text-[14px]">stoke not Enif</p>
                  ) : (
